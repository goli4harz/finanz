...
- **Nachtrag A10 (2026-08-02):** Optimistic Locking über das bereits vorhandene, nie genutzte `version`-Feld. „Baue HTML" bettet den Wert als verstecktes Formularfeld ein, „POST: Baue Load-Query"/„POST: Formular normalisieren + SQL bauen" vergleichen ihn gegen den frisch geladenen DB-Wert - bei Abweichung kein Schreibversuch. Jedes Status-Update inkrementiert `version` mit. Abwärtskompatibel bei fehlendem Feld. `DB: Vorschlaege laden` liefert `version` für die Liste mit. Migration: keine (Spalte existiert seit `sql/001`). Reimport: bereits live gepusht (Backup `n8n_live_backup/12_-_Lernvorschlag-Freigabe_PRE_A10_20260802_010102.json`).
