...
Vollständiger Audit + Konzeptbericht "Persönlicher KI-Trading-Analyst" (2026-08-24, Artefakt-Link in `docs/`-Historie/Memory, nicht im Repo) — zentrale Idee: eine neue **Historical Evidence Engine**, historische News-Wirkungsfälle aktiv fürs Matching neuer Alerts nutzbar machen, plus ein zentrales Actionability Gate (ALERT/WATCH/INFO/IGNORE) statt der heutigen mehrfach unabhängig alarmierenden Workflows. Umsetzungsreihenfolge aus Kap. 18 des Konzepts wird seitdem Schritt für Schritt abgearbeitet:

1. ✅ **Ist-Zustand bereinigt** (2026-08-24): `ENABLE_PAPER_TRADING=true` als beabsichtigt bestätigt; `REQUIRE_CONFIRMATION` war seit 2026-07-20 undokumentiert auf `false` gekippt — live zurückgesetzt auf `true`; Entscheidung getroffen, den `probability_estimates`-Producer schon in V1 zu bauen (Konzept empfahl eigentlich V2).
2. ✅ **`probability_estimates`-Producer gebaut** (2026-08-25/27/28): Workflow `19 – Wahrscheinlichkeitskalibrierung (Simulation)`, live und Ende-zu-Ende verifiziert (2026-08-29) — Trading-Entscheidungszentrale matcht Empfehlungen jetzt gegen echte Stichproben. Dabei mehrere Bugs gefunden+gefixt: `market_regime_at_entry` wurde vom Legacy-Pfad in Workflow 17 nie berechnet (Regime-Logik nachgezogen), Sim-Exit-Logik hatte zwei strukturelle Fehler (Trailing-Stop fälschlich auch für mean_reversion, Zeitstop in Kalender- statt Handelstagen), MR-Einstieg bekam einen Primärtrend-Filter. Ergebnis nach Neu-Kalibrierung: mean_reversion ist über alle Regime hinweg ein Nettoverlierer — valides Kalibrierungsergebnis, kein Bug.
3. ✅ **`historical_news`-Zukunftsdatum-Bug gefixt** (2026-08-29, `ab67c28`): einzelne GDELT-Zeile mit `published_at 2026-12-31`, `sanitizePublishedAt()` in Workflow 16 ergänzt (Fallback auf Importzeit statt roher Quellenangabe).
4. ✅ **Workflow-08 D+10/D+20 operativ geprüft** (2026-08-30, `988b4a4`): kein Logikbug in `08` selbst — `stock_price_history` hat eine ~19-Handelstage-Lücke durch den n8n-Ausfall vor 08-17 (siehe Abschnitt zum Instanzumzug), `08` überspringt die Lücke stillschweigend und verankert D+10/D+20 an falschen Handelstagen. Fix: neue `hasStaleDataGap()`-Prüfung markiert betroffene Zeilen als `confounded` statt sie falsch auszuwerten (142 von 824 wartenden Zeilen betroffen). **Noch nicht gegen einen echten Lauf verifiziert** — nächster Schritt: nach dem nächsten Werktags-19:00-Lauf prüfen, ob betroffene Zeilen tatsächlich auf `confounded` kippen.
5. ✅ **`v_news_evidence_cases`-View live** (2026-08-30, `sql/075`, über den Workflow-97-Diagnose-Webhook auf der neuen Instanz `172.16.1.6:5678` ausgeführt und verifiziert): reine Lesesicht auf `v_news_impact_tracking_combined` + Sektor + Marktregime zum Ereigniszeitpunkt, wie in Kap. 9/18 des Konzepts spezifiziert (mit einer Korrektur ggü. dem Rohentwurf: `region_at_event` aus `stock_instruments.currency` abgeleitet statt der im Entwurf skizzierten, nicht existierenden Konstante `'DE'`). Live-Check: 657 Zeilen (alle `historical`, 2025-03-14..2026-01-10), alle mit `sektor` befüllt. **Neuer Fund dabei**: `trading.market_regime` hat aktuell nur 15 Zeilen (2026-08-21..2026-08-28) — keine historische Rückrechnung. `regime_at_event` ist damit für praktisch alle bestehenden Fälle NULL (ehrlich, kein erfundener Wert), beantwortet die im Audit selbst offen gelassene Frage aus Kap. 9 ("per-Tag-Historie wirklich vorhanden?") für den heutigen Datenbestand mit strukturell ja, faktisch nein. Für künftige Live-Ereignisse (ab 2026-08-21) funktioniert der Join bereits; ein Backfill von `market_regime` wäre nötig, damit die historischen Fälle ebenfalls ein Regime bekommen — noch kein eigener Punkt, siehe nächster Abschnitt.
6. ✅ **Historical-Evidence-Lookup-Subworkflow gebaut + isoliert getestet** (2026-08-30, `sql/076` + Workflow `Historical Evidence Lookup`, live id `14tavTv1i5cuTr0U`): Execute-Workflow-Subworkflow (Input: `ticker`, `sektor`, `news_category`, `as_of_date`, `news_id`), implementiert die Matching-Hierarchie aus Kap. 9 (Ticker+Kategorie → Ticker → Sektor+Kategorie → Sektor → Gesamtbestand) gegen `v_news_evidence_cases` — erste Stufe mit ≥1 nicht-konfundiertem Fall wird verwendet, `evidence_sufficient` markiert ehrlich ob die Mindeststichprobe (10 auf Ticker-, 20 auf Sektor-/Gesamtbestand-Ebene) erreicht ist, sonst wörtlich "Historische Evidenz nicht ausreichend" (nie erfundene Werte). Statistik je Horizont (D+1/3/5/10/20): n, Median-/Durchschnitts-Return, durchschnittliche abnormale (benchmark-relative) Rendite, Richtungstreffer-Quote — konfundierte Fälle separat gezählt, nie in die Statistik gemischt. Jede Anfrage wird in der neuen Tabelle `trading.news_evidence_lookups` protokolliert. Live über den WF97-Diagnose-Webhook getestet: Tier-1-Fall (BMW.DE/Auto/other, n=20, konfundiert=35) und Tier-5-Fallback (erfundener Ticker, faellt bis zum Gesamtbestand durch, n=158) beide korrekt. **Noch NICHT an Workflow 03 angebunden** (das ist laut Plan der nächste, separate Schritt) — der Subworkflow existiert und funktioniert isoliert, wird aber von nichts aufgerufen.
7. ✅ **Historical-Evidence-Lookup an Workflow 03 angebunden** (2026-08-30, zunächst nur protokollierend, kein Alert-Effekt): Subworkflow-Interface vereinfacht — `sektor` wird jetzt intern per Ticker aus `stock_instruments` nachgeschlagen statt vom Aufrufer übergeben (Retest mit BMW.DE bestätigt identisches Ergebnis wie zuvor). In `03` als **rein additiver Parallelzweig** direkt nach `KI-Bewertung aufbereiten` eingehängt: `Filter: Relevante mit Ticker (Evidence-Lookup)` (nur `_action='persist'`, `relevant=true`, mind. 1 bestätigter Watchlist-Ticker) → `Explodiere je Ticker` (1 Lookup-Aufruf pro betroffenem Ticker, `news_category`=`news_kategorie`, `as_of_date=null` für Live) → `Rufe Historical Evidence Lookup auf` (`onError:continueRegularOutput`, damit ein Lookup-Fehler nie den bestehenden News-Ingestion-Lauf gefährdet). Strukturell verifiziert: alle 38 bisherigen Knoten byte-identisch, nur 3 neue Knoten + 1 neuer Ausgang an der bestehenden `KI-Bewertung aufbereiten`-Verzweigung (die beiden alten Ziele `Ergebnis persistieren`/`Batch-Bewertung protokollieren` unverändert). **Noch nicht gegen einen echten stündlichen Lauf verifiziert** — nächster Schritt: nach dem nächsten `03`-Lauf prüfen, ob `trading.news_evidence_lookups` neue Zeilen bekommt.
8. ✅ **Actionability-Gate-Subworkflow gebaut + isoliert getestet** (2026-08-30, `sql/077` + Workflow `Actionability Gate`, live id `1168c8jXJfvYLz60`): Execute-Workflow-Subworkflow mit den 4 Klassen aus Kap. 13 (IGNORE → INFO → WATCH → ALERT, in dieser Reihenfolge geprüft). `computed_decision` hält fest, was die Regellogik ergeben hätte (kann schon `ALERT` sein), `decision` ist der tatsächlich wirksame, laut Kap. 18 Schritt 7 auf `WATCH` gedeckelte Wert (`alert_capped`-Flag) — die Deckelung lässt sich in Schritt 9 aufheben, ohne die Entscheidungslogik selbst anzufassen. Jede Entscheidung wird in der neuen Tabelle `trading.actionability_decisions` protokolliert. **5 Testfälle live über WF97 verifiziert** (alle korrekt): kein technisches Signal → `WATCH`; starkes Signal + ausreichende Evidenz + kein Veto → Logik ergibt `ALERT`, aber gedeckelt auf `WATCH`; Portfolio-Veto aktiv → blockiert `ALERT` unabhängig von Evidenzqualität (`WATCH`); nicht relevant → `IGNORE`; kein Tickerbezug (Makro-News) → `INFO`. **Noch NICHT an 03/06/14/05 angebunden** — der Subworkflow existiert und funktioniert isoliert, die eigentliche Verdrahtung (wer ruft ihn mit welchen Inputs auf, insbesondere `hard_veto_active`/`regime_fit_ok`/`technical_confirmation` aus den bestehenden Systemen) ist laut Plan ein späterer Schritt.
9. ✅ **Actionability Gate an Workflow 06 angebunden** (2026-08-30, Kap. 18 Schritt 9 teilweise vorgezogen, weiterhin nur protokollierend/gedeckelt): Kernknoten `Empfehlungen: Abgleich berechnen` prüft harte Vetos und Regime-Blockaden bereits VOR der Erzeugung eines `oeffnen`-Kandidaten (Treffer erzeugen nur einen `veto_log`-Eintrag, kein `oeffnen`-Item) — dadurch gilt für jeden hier ankommenden Kandidaten strukturell schon `hard_veto_active=false`/`regime_fit_ok=true`, ohne das erneut prüfen zu müssen. Minimal-additive Ergänzung: neues Feld `technical_confirmation: !trendKonflikt` im bestehenden `oeffnen`-Item (44 Zeichen Diff, sonst byte-identischer Code, strukturell verifiziert). Neuer Parallelzweig (5 Knoten, dead-end): Filter auf `_aktion==='oeffnen'` → Historical-Evidence-Lookup pro Kandidat (Tier 2, da Kandidaten keine feste News-Kategorie haben) → Ableitung der Actionability-Gate-Eingaben (inkl. `evidence_conflicting`-Heuristik: D+5-Richtungstrefferquote < 0.4) → Actionability-Gate-Aufruf. Bewusste Vereinfachungen dieser ersten Stufe dokumentiert (im Code-Kommentar): `is_duplicate`/`is_cooldown_active` immer `false` (kein Cooldown-Konzept), `data_quality_ok` immer `true` (strukturell schon gefiltert), `recommendation_id` immer `null` (Kandidat hat zu diesem Zeitpunkt noch keine DB-ID). Strukturell verifiziert: alle 66 bisherigen Knoten bis auf den einen erwähnten Kernknoten byte-identisch, nur 5 neue Knoten + 1 neuer Ausgang, Syntax aller neuen/geänderten JS-Codes geprüft. **Workflow 06 läuft nur Mo-Fr 18:10 — noch nicht gegen einen echten Lauf verifiziert** (wie WF03, das ebenfalls nur Mo-Fr läuft), nächster Schritt Montag.
10. ✅ **Actionability Gate auch an Workflow 14 angebunden** (2026-08-30, schließt die in Schritt 9 dokumentierte Lücke): `Job A: Portfoliopruefung + Trade-Anlage` schreibt pro Kandidat einen `_typ:'recommendation_status_update'`-Eintrag mit `new_status` (`offen`/`portfolio_blocked`/`portfolio_check_failed`) und der **echten** `recommendation_id` (existiert hier bereits als DB-Zeile aus 06). Zweiter, additiver Parallelzweig (5 neue Knoten, dead-end) ruft Historical Evidence Lookup + Actionability Gate erneut auf, diesmal mit `hard_veto_active = (new_status !== 'offen')` — deckt also die Portfolio-Ebene-Vetos ab, die der WF06-Aufruf strukturell nicht sehen kann. Beide Aufrufe (06 früh im Signal, 14 nach Portfolio-Prüfung) ergänzen sich zu einem vollständigeren Bild, ohne dass einer den anderen ersetzt. **Diesmal kein einziger bestehender Knoten verändert** (0 Byte Diff) — noch minimal-invasiver als die WF06-Anbindung, da `Job A` die benötigten Felder (`ticker`/`new_status`/`recommendation_id`) bereits ohne Änderung liefert. Strukturell verifiziert + JS-Syntax geprüft. **Workflow 14 läuft nur Mo-Fr 18:15 — noch nicht gegen einen echten Lauf verifiziert**, gleiche Situation wie 03/06.
11. ⬜ **Restpunkte aus dem 08-24-Audit**, noch nicht angegangen: `docs/WELLE_3_ABGLEICH...`-artige neue Doku für die Evidence Engine, spätere Kalibrierung der Evidence-Matching-Trefferquote selbst (V3-Thema).

## Historische Nachrichten: Import-Stillstand + Filter-Bereinigung (2026-08-30)

Auf Nutzerfrage ("kann ich noch etwas machen? Nachrichten importieren?") geprüft:

- ⬜ **Kein Nachrichten-Import-Job seit 2026-08-22** (`trading.import_jobs`, letzter `job_type='news'` deckte nur bis `period_to=2026-08-16` ab). `16 – Historische Nachrichten importieren (GDELT)` hat zwar einen 5-Minuten-Cron, tut aber ohne aktiven Job nichts — daher praktisch keine Ausführungen sichtbar. `16b – Historische Nachrichten bewerten (KI)` hat gar keinen eigenen Trigger, wird nur von `16` nach einem Import aufgerufen (max. 10 News pro Aufruf). **Nutzer kann selbst einen neuen Import-Job starten**: `http://172.16.1.6:5678/webhook/historische-nachrichten` (Zeitraum 17.08.–heute fehlt).
- ✅ **4.795 permanent gefilterte `historical_news`-Zeilen gelöscht** (2026-08-30, Nutzerentscheidung: "wenn Nachrichten durch den Filter nicht mehr verarbeitet werden, sollten sie gelöscht werden"): von den 9.862 als "pending" gezählten Zeilen hatten 48,6% (`ticker_match_status='none'` UND `is_general_market=false`) strukturell keine Chance, jemals von 16b aufgegriffen zu werden (dessen eigene Kandidaten-Abfrage filtert exakt darauf). Vor dem Löschen FK-Abhängigkeiten geprüft: nur `news_match_exclusion_hits` (4.841 Zeilen, NO ACTION) betroffen, mit gelöscht; `historical_news_assessments`/`historical_news_impact_tracking` hatten 0 Treffer (unbewertete News, unproblematisch). Verifiziert: `historical_news` 28.832→24.037, `pending` 9.862→5.067 (= der tatsächlich bearbeitbare Rest), `assessed` unverändert 18.970.
- ⬜ **Das war eine einmalige Bereinigung, keine automatische Regel.** Neue GDELT-Importe werden weiterhin einen Anteil `ticker_match_status='none'`-News ohne Marktbezug produzieren, die sich wieder ansammeln. Falls gewünscht: eigener wiederkehrender Cleanup-Workflow (analog ALLRIS' `04 – Cleanup News-Tabellen`) wäre ein separates, noch nicht beauftragtes Vorhaben.
- ⬜ **Restlicher echter Rückstand: 5.067 unbewertete, aber grundsätzlich bearbeitbare News.** Abarbeitung braucht entweder einen neuen Import-Job (löst 16b automatisch mit aus) oder ein manuelles Ausführen von 16b — Hinweis an den Nutzer: das sind reale KI-Aufrufe (Kosten/Zeit), nicht nebenbei mal eben anzustoßen.

**Nebenbei erledigt in diesem Zeitraum:** WF17-Worker hing bei datenlosen Sim-Tagen endlos (gefixt+verifiziert, `2984811`); POST-Aktionen in WF17/WF12/16c/16d auf projektweiten Re-Render-statt-Zwischenseite-Standard vereinheitlicht; "Lauf löschen"-UI für Simulationsläufe gebaut.

**Instanzumzug (bestätigt 2026-08-27, IP erneut geändert 2026-08-30):** Finanz-n8n lief bis 2026-08-27 auf der gemeinsamen ALLRIS-Box `172.16.1.14:5678` (~24 Workflows dort am 2026-08-27 bulk-archiviert), danach auf einer eigenen Instanz zunächst unter `172.16.1.17:5678`, seit 2026-08-30 unter neuer IP `172.16.1.6:5678` (vom Nutzer mitgeteilt, per API bestätigt — 118 Workflows dort, u.a. auch alte ALLRIS-„Einmalig"-Diagnose-Workflows, scheint also eine konsolidierte Box zu sein, nicht rein finanz-exklusiv). Repo-Live-Abgleich war zuletzt am 2026-08-23 gegen die alte `.14` gemacht worden — ein erneuter vollständiger Abgleich ist grundsätzlich überfällig, auch wenn die seitherigen Sessions einzelne Workflows gezielt synchronisiert haben.
...
**⚠️ Zweiter Sonderfall (Härtung Welle 1-3, Phase 14, 2026-08-02): `00 – Tagesabschluss-Orchestrator.json` weicht bewusst von Live ab.** Der Repo-Stand enthält die vollständige Verdrahtung von `13` (Markt-Screener) und `14` (Portfolio-Risiko/Paper-Trading) inkl. Feature-Flag-Gates (`ENABLE_MARKET_SCANNER`/`ENABLE_PAPER_TRADING`, beide `FALSE` in der DB), ist aber **absichtlich nicht live gepusht** — n8n verweigert das Speichern eines Workflows, der einen inaktiven Sub-Workflow per `executeWorkflow` referenziert (`HTTP 400: "which is not published"`, verifiziert über `activeVersionId`). Da `13`/`14` laut Sicherheitsregel vor bestandenen Tests nicht aktiviert werden dürfen, kann `00`s neue Version erst zusammen mit deren kontrollierter Erstaktivierung (Phase 18, Aktivierungsplan) live gepusht werden. Die aktuell laufende Live-Version von `00` (ohne 13/14-Stufen) bleibt bis dahin unverändert aktiv und funktionsfähig. Vor jedem GET-Resync von `00` das PRE_PHASE14-Backup bzw. `FEHLERANALYSE_HAERTUNG_WELLE_1_3.md` (Phase 14) lesen.

**✅ Härtungsauftrag "Vollständige Härtung und betriebsfähige Integration" (Welle 1-3, 18 Phasen) abgeschlossen, 2026-08-02.** Siehe `ABSCHLUSSBERICHT_HAERTUNG_WELLE_1_3.md` für die Gesamtempfehlung, `AKTIVIERUNGSPLAN_PAPER_TRADING.md` für die nächsten Schritte (Scanner-Aktivierung Stufe 1), `PRODUKTIONSFREIGABE_PAPER_TRADING.md` für die Ampel-Bewertung je Komponente und `AENDERUNGSPROTOKOLL_HAERTUNG_WELLE_1_3.md` für die kompakte Liste aller geänderten Dateien. Die beiden Sonderfälle oben (`06`, `00`) bleiben bis zur jeweiligen Aktivierungsstufe bestehen.
...
- 🟢 **AP8** (Kalibrierung): Schema+Formeln vollständig spezifiziert (`sql/037`). `paper_trades`-Quelle weiterhin **nicht gebaut** (0 abgeschlossene Paper Trades). **Nachtrag 2026-08-25**: für die `simulation_trades`-Quelle (sql/071, `data_source='simulation_trades'`) gebaut und live — Workflow `19 – Wahrscheinlichkeitskalibrierung (Simulation)`, dazu Point-in-time-Marktregime in Workflow `17` und Konsument (`Trading-Entscheidungszentrale.json`) auf `LEFT JOIN LATERAL` umgestellt. **Nachtrag 2026-08-27**: Repo-Drift behoben (lokale Arbeitskopie war nie committet, live aber schon länger aktuell — Workflow `17` seither zusätzlich um Signal-Batching wegen Performance-Problemen erweitert). Noch offen: Vorbedingungs-Check (7 Regime-Referenzticker in `historical_price_data`), Testlauf gegen eine echte Empfehlung. Siehe `docs/WAHRSCHEINLICHKEITSKALIBRIERUNG.md`.
