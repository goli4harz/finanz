...
5. 🟡 **`v_news_evidence_cases`-View entworfen, noch nicht live** (2026-08-30, `sql/075`): reine Lesesicht auf `v_news_impact_tracking_combined` + Sektor + Marktregime zum Ereigniszeitpunkt, wie in Kap. 9/18 des Konzepts spezifiziert (mit einer Korrektur ggü. dem Rohentwurf: `region_at_event` aus `stock_instruments.currency` abgeleitet statt der im Entwurf skizzierten, nicht existierenden Konstante `'DE'`). **Blockiert auf Live-Ausführung**: die Finanz-n8n-Instanz lief seit 2026-08-27 auf einer eigenen Instanz `172.16.1.17:5678` (siehe unten), der in dieser Umgebung hinterlegte `N8N_API_KEY` gehört noch zur alten gemeinsamen ALLRIS-Instanz `.14` (401 gegen `.17`) — braucht entweder einen aktuellen `.17`-Key oder eine manuelle Ausführung über Workflow `97`.
6. ⬜ **Noch offen** (nächste Schritte laut Kap. 18, nach `v_news_evidence_cases`): Historical-Evidence-Lookup-Subworkflow bauen + isoliert testen, an Workflow 03 anbinden (zunächst nur protokollierend), Actionability-Gate-Subworkflow, Decision-Inbox-Umbau der Startseite, `v_news_evidence_cases` in Workflow 17 statt (bzw. zusätzlich zu) direkter Logik nutzen.
...
**Instanzumzug (bestätigt 2026-08-27):** Finanz-n8n lief bis dahin auf der gemeinsamen ALLRIS-Box `172.16.1.14:5678` (~24 Workflows dort am 2026-08-27 bulk-archiviert), läuft seitdem auf einer eigenen Instanz `172.16.1.17:5678`. Repo-Live-Abgleich gegen `.17` war zuletzt am 2026-08-23 gegen `.14` gemacht worden — ein erneuter Abgleich ist grundsätzlich überfällig, auch wenn die seitherigen Sessions einzelne Workflows gezielt synchronisiert haben.
