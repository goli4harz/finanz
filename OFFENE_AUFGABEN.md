...
6. ⬜ **Noch offen** (nächste Schritte laut Kap. 18, nach `v_news_evidence_cases`): Historical-Evidence-Lookup-Subworkflow bauen + isoliert testen, an Workflow 03 anbinden (zunächst nur protokollierend), Actionability-Gate-Subworkflow, Decision-Inbox-Umbau der Startseite, `v_news_evidence_cases` in Workflow 17 statt (bzw. zusätzlich zu) direkter Logik nutzen. Neu dazugekommen: `market_regime`-Backfill klären (siehe Fund oben) — betrifft sowohl diese View als auch, ob es für ältere `simulation_trades`/`recommendations` je ein echtes Regime statt `unknown` geben könnte.
7. ⬜ **Restpunkte aus dem 08-24-Audit**, noch nicht angegangen: `docs/WELLE_3_ABGLEICH...`-artige neue Doku für die Evidence Engine, spätere Kalibrierung der Evidence-Matching-Trefferquote selbst (V3-Thema).
